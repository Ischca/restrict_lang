...
#[test]
fn dogfood_result_local_inference_example_type_checks() {
    let source = include_str!("../examples/dogfood_result_local_inference.rl");

    type_check_source(source).expect("local Result inference dogfood should type-check");
}

#[test]
fn dogfood_result_local_inference_example_generates_valid_wat() {
    let source = include_str!("../examples/dogfood_result_local_inference.rl");
    let wat = compile_to_wat(source).expect("local Result inference dogfood should compile to WAT");

    assert!(
        wat.contains("(func $route_score"),
        "WAT should contain the Result route scorer:\n{wat}"
    );
    assert!(
    ...
    let wasm = wat::parse_str(&wat).unwrap_or_else(|err| {
        panic!("local Result inference dogfood generated invalid WAT: {err}\n\n{wat}");
    });
    wasmparser::Validator::new()
        .validate_all(&wasm)
        .unwrap_or_else(|err| {
            panic!("local Result inference dogfood generated invalid Wasm binary: {err}\n\n{wat}");
        });
}

#[test]
fn dogfood_result_local_inference_example_executes() {
    let source = include_str!("../examples/dogfood_result_local_inference.rl").replace(
        "fun main: () -> Int32",
        "export fun result_local_score: () -> Int32",
    );
    let wat = compile_to_wat(&source)
        .expect("local Result inference dogfood runtime wrapper should compile to WAT");

    let (mut store, instance) = instantiate_wat("local Result inference dogfood runtime", &wat);
    let result_local_score = instance
        .get_typed_func::<(), i32>(&store, "result_local_score")
