...
#[test]
fn dogfood_nested_result_list_inference_example_type_checks() {
    let source = include_str!("../examples/dogfood_nested_result_list_inference.rl");

    type_check_source(source)
        .expect("nested Result<List<T>> generic record dogfood should parse and type-check");
}

#[test]
fn dogfood_nested_result_list_inference_example_generates_valid_wat() {
    let source = include_str!("../examples/dogfood_nested_result_list_inference.rl");
    let wat = compile_to_wat(source).expect("nested Result<List<T>> dogfood should compile to WAT");

    assert!(
        wat.contains("(func $dogfood_nested_result_list_score"),
        "WAT should contain the exported billing import score:\n{wat}"
    );
    assert!(
    ...
    assert!(
        ...
        "inferred Result<List<Int64>> payload should store Int64 list elements:\n{wat}"
    );
    assert!(
        wat.contains("call $billable_delta_score"),
        "later match payload use should call the List<Int64> scoring helper:\n{wat}"
    );
    assert!(
    ...
    let wasm = wat::parse_str(&wat).unwrap_or_else(|err| {
        panic!("nested Result<List<T>> dogfood generated invalid WAT: {err}\n\n{wat}");
    });
    wasmparser::Validator::new()
        .validate_all(&wasm)
        .unwrap_or_else(|err| {
            panic!("nested Result<List<T>> dogfood generated invalid Wasm binary: {err}\n\n{wat}");
        });
}

#[test]
fn dogfood_nested_result_list_inference_example_executes() {
    let source = include_str!("../examples/dogfood_nested_result_list_inference.rl");
    let wat = compile_to_wat(source)
        .expect("nested Result<List<T>> dogfood runtime export should compile to WAT");

    let (mut store, instance) = instantiate_wat("nested Result<List<T>> dogfood runtime", &wat);
    let score = instance
        .get_typed_func::<(), i64>(&store, "dogfood_nested_result_list_score")
