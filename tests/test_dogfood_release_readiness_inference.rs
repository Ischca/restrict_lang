...
#[test]
fn dogfood_release_readiness_inference_example_type_checks() {
    let source = include_str!("../examples/dogfood_release_readiness_inference.rl");

    type_check_source(source)
        .expect("release readiness dogfood example should parse and type-check");
}

#[test]
fn dogfood_release_readiness_inference_example_generates_valid_wat() {
    let source = include_str!("../examples/dogfood_release_readiness_inference.rl");
    let wat =
        compile_to_wat(source).expect("release readiness dogfood example should compile to WAT");
    ...
    let wasm = wat::parse_str(&wat).unwrap_or_else(|err| {
        panic!("release readiness dogfood example generated invalid WAT: {err}\n\n{wat}");
    });
    wasmparser::Validator::new()
        .validate_all(&wasm)
        .unwrap_or_else(|err| {
            panic!(
                "release readiness dogfood example generated invalid Wasm binary: {err}\n\n{wat}"
            );
        });
}

#[test]
fn dogfood_release_readiness_inference_example_executes() {
    let source = include_str!("../examples/dogfood_release_readiness_inference.rl").replace(
        "fun main: () -> Int32",
        "export fun release_readiness_score: () -> Int32",
    );
    let wat = compile_to_wat(&source)
        .expect("release readiness dogfood runtime wrapper should compile to WAT");

    let (mut store, instance) = instantiate_wat("release readiness dogfood runtime", &wat);
