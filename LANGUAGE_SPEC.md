...
### 3.3 パイプ演算子
- `|>`: 不変パイプ（単一の標準パイプ演算子）
- 左辺の値を右辺の関数の第1引数として渡す
- 多引数関数にはタプルを使用し、パイプ時に自動展開される
...
### 3.4 タプル型
- タプル型: `(T1, T2, ..., Tn)` — 固定長の異種コレクション
- タプルリテラル: `(expr1, expr2, ...)`
- `()` は Unit型
- `(expr)` は括弧つき式（タプルではない）
- パイプ時にタプルの要素が関数の引数に自動展開される
- 要素数と関数の引数の数が一致しない場合はコンパイルエラー
- 各要素はアフィン型制約に従う
