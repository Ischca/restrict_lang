...
#[test]
fn dogfood_generic_pipeline_runtime_example_type_checks() {
    let source = include_str!("../examples/dogfood_generic_pipeline_runtime.rl");

    type_check_source(source).expect("generic pipeline runtime dogfood should type-check");
}

#[test]
fn dogfood_generic_pipeline_runtime_example_generates_valid_wat() {
    let source = include_str!("../examples/dogfood_generic_pipeline_runtime.rl");
    let wat = compile_to_wat(source).expect("generic pipeline runtime dogfood should compile");
    ...
    assert!(
        wat.contains("(func $identity_local"),
        "WAT should contain the generic identity specialization target:\n{wat}"
    );
    assert!(
        wat.contains("(func $choose_first"),
        "WAT should contain the generic chooser specialization target:\n{wat}"
    );

    let wasm = wat::parse_str(&wat).unwrap_or_else(|err| {
        panic!("generic pipeline runtime dogfood generated invalid WAT: {err}\n\n{wat}");
    });
    wasmparser::Validator::new()
        .validate_all(&wasm)
        .unwrap_or_else(|err| {
            panic!(
                "generic pipeline runtime dogfood generated invalid Wasm binary: {err}\n\n{wat}"
            );
        });
}

#[test]
fn dogfood_generic_pipeline_runtime_example_executes() {
    let source = include_str!("../examples/dogfood_generic_pipeline_runtime.rl");
    let wat = compile_to_wat(source).expect("generic pipeline runtime dogfood should compile");

    let (mut store, instance) = instantiate_wat("generic pipeline runtime dogfood", &wat);
    let score = instance
        .get_typed_func::<(), i32>(&store, "dogfood_generic_pipeline_runtime_score")
