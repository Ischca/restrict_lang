...
execution uses a no-result `_start` wrapper only when `main` takes no
parameters. The wrapper initializes the default arena, calls `$main`, drops any
returned value, and resets the arena.
