...
#[test]
fn dogfood_bug_triage_board_runtime_example_type_checks() {
    let source = include_str!("../examples/dogfood_bug_triage_board_runtime.rl");

    type_check_source(source).expect("bug triage board runtime dogfood should type-check");
}

#[test]
fn dogfood_bug_triage_board_runtime_example_generates_valid_wat() {
    let source = include_str!("../examples/dogfood_bug_triage_board_runtime.rl");
    let wat = compile_to_wat(source).expect("bug triage board runtime should compile to WAT");

    assert!(
        wat.contains("(func $bug_triage_board_runtime_score"),
        "WAT should contain the host-callable runtime wrapper:\n{wat}"
    );
    assert!(
    ...
    let wasm = wat::parse_str(&wat).unwrap_or_else(|err| {
        panic!("bug triage board runtime dogfood generated invalid WAT: {err}\n\n{wat}");
    });
    wasmparser::Validator::new()
        .validate_all(&wasm)
        .unwrap_or_else(|err| {
            panic!(
                "bug triage board runtime dogfood generated invalid Wasm binary: {err}\n\n{wat}"
            );
        });
}

#[test]
fn dogfood_bug_triage_board_runtime_example_executes() {
    let source = include_str!("../examples/dogfood_bug_triage_board_runtime.rl");
    let wat =
        compile_to_wat(source).expect("bug triage board runtime wrapper should compile to WAT");

    let (mut store, instance) = instantiate_wat("bug triage board runtime dogfood", &wat);
    let bug_triage_board_runtime_score = instance
        .get_typed_func::<(), i32>(&store, "bug_triage_board_runtime_score")
        .expect("runtime wrapper should be host-callable");

    assert_eq!(
        bug_triage_board_runtime_score
            .call(&mut store, ())
