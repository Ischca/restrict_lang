...
fn dogfood_examples_have_dedicated_tests_or_explicit_gap_entries() {
    ...
    for example in &examples {
        ...
        let expected_test_path = root.join("tests/cases").join(&expected_test);
        let test_source = match fs::read_to_string(&expected_test_path) {
            Ok(source) => source,
            Err(err) => {
                missing.push(format!(
                    "examples/{example} -> missing tests/cases/{expected_test}: {err}"
                ));
                continue;
            }
        };
        ...
        if !test_source.contains(&expected_reference) {
            missing.push(format!(
                "examples/{example} -> tests/cases/{expected_test} does not reference {expected_reference:?}"
            ));
        }

        if !has_runtime_execution_coverage(&test_source) {
            missing.push(format!(
                "examples/{example} -> tests/cases/{expected_test} should execute generated Wasm \
