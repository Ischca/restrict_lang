...
The source entry point is `main`. During code generation, `main` is called by a
host `_start` wrapper. If `main` returns a value, the wrapper drops it for
program-style execution; expose host-callable scalar results through a separate
`pub fun` or `export fun`.
