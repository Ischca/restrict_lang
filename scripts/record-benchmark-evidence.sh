...
RUN_COUNT="${2:-5}"
STABILITY_POLICY="${3:-benchmarks/stability-policy.json}"
...
if [[ ! -f "$STABILITY_POLICY" ]]; then
  echo "stability policy not found: $STABILITY_POLICY" >&2
  exit 2
fi

for executable in \
  ./target/release/restrict_bench \
  ./target/release/restrict_bench_compare \
  ./target/release/restrict_bench_stability; do
