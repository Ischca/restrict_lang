...
#[test]
fn dogfood_branch_callable_prefix_inference_example_type_checks() {
    let source = include_str!("../examples/dogfood_branch_callable_prefix_inference.rl");

    type_check_source(source)
        .expect("branch callable prefix dogfood example should parse and type-check");
}

#[test]
fn dogfood_branch_callable_prefix_inference_example_generates_valid_wat() {
    let source = include_str!("../examples/dogfood_branch_callable_prefix_inference.rl");
    let wat = compile_to_wat(source).expect("branch callable prefix dogfood should compile to WAT");
    ...
    assert!(
        ...
        "WAT should use callable values for branch-produced mappers:\n{wat}"
    );
    wat::parse_str(&wat)
        .unwrap_or_else(|err| panic!("dogfood example generated invalid WAT: {err}\n\n{wat}"));
}

#[test]
fn dogfood_branch_callable_prefix_inference_example_executes() {
    let source = include_str!("../examples/dogfood_branch_callable_prefix_inference.rl").replace(
        "fun main: () -> Int32",
        "export fun branch_callable_prefix_score: () -> Int32",
    );
    let wat = compile_to_wat(&source)
        .expect("branch callable prefix dogfood runtime wrapper should compile to WAT");

    let (mut store, instance) = instantiate_wat("branch callable prefix dogfood runtime", &wat);
    let branch_callable_prefix_score = instance
        .get_typed_func::<(), i32>(&store, "branch_callable_prefix_score")
        .expect("runtime wrapper should be host-callable");

    assert_eq!(
        branch_callable_prefix_score
            .call(&mut store, ())
