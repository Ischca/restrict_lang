...
#[test]
fn dogfood_incident_triage_runtime_example_type_checks() {
    let source = include_str!("../examples/dogfood_incident_triage_runtime.rl");

    type_check_source(source).expect("incident triage runtime dogfood should type-check");
}

#[test]
fn dogfood_incident_triage_runtime_example_generates_valid_wat() {
    let source = include_str!("../examples/dogfood_incident_triage_runtime.rl");
    let wat = compile_to_wat(source).expect("incident triage runtime should compile to WAT");

    assert!(
        wat.contains("(func $incident_triage_runtime_score"),
    ...
    let wasm = wat::parse_str(&wat).unwrap_or_else(|err| {
        panic!("incident triage runtime dogfood generated invalid WAT: {err}\n\n{wat}");
    });
    wasmparser::Validator::new()
        .validate_all(&wasm)
        .unwrap_or_else(|err| {
            panic!("incident triage runtime dogfood generated invalid Wasm binary: {err}\n\n{wat}");
        });
}

#[test]
fn dogfood_incident_triage_runtime_example_executes() {
    let source = include_str!("../examples/dogfood_incident_triage_runtime.rl");
    let wat =
        compile_to_wat(source).expect("incident triage runtime wrapper should compile to WAT");

    let (mut store, instance) = instantiate_wat("incident triage runtime dogfood", &wat);
    let incident_triage_runtime_score = instance
        .get_typed_func::<(), i32>(&store, "incident_triage_runtime_score")
        .expect("runtime wrapper should be host-callable");

    assert_eq!(
        incident_triage_runtime_score
            .call(&mut store, ())
