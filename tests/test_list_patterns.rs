...
fn test_list_pattern_exact() {
    let input = "fun main = { with Arena { val lst = [1, 2, 3] val result = (lst) match { [] => { 0 } [a] => { a } _ => { 99 } } result } }";
    let wat = compile(input).unwrap();
    assert!(wat.contains("i32.load")); // Check for length load
...
fn test_list_pattern_cons() {
    let input = "fun main = { with Arena { val lst = [10, 20, 30] val result = (lst) match { [] => { 0 } [head | tail] => { head } _ => { -1 } } result } }";
    let wat = compile(input).unwrap();
...
fn test_list_pattern_nested() {
    let input = "fun main = { with Arena { val lst = [1, 2] val s = (lst) match { [] => { 0 } [h | t] => { h + (t) list_length } _ => { 0 } } s } }";
    let wat = compile(input).unwrap();
...
fn test_list_pattern_multiple_elements() {
    let input = "fun main = { with Arena { val lst = [1, 2, 3, 4] val result = (lst) match { [] => { 0 } [h | t] => { h } _ => { 99 } } result } }";
    let wat = compile(input).unwrap();
