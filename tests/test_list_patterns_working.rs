use restrict_lang::{generate, parse_program, TypeChecker};

fn compile_simple(source: &str) -> Result<String, String> {
    let trimmed = source.trim();

    let (remaining, ast) = parse_program(trimmed).map_err(|e| format!("Parse error: {:?}", e))?;

    // Check if we parsed any declarations
    if ast.declarations.is_empty() && !trimmed.is_empty() {
        return Err("No declarations parsed from non-empty input".to_string());
    }

    if !remaining.trim().is_empty() {
        return Err(format!("Unparsed input: {:?}", remaining));
    }

    let mut type_checker = TypeChecker::new();
    type_checker
        .check_program(&ast)
        .map_err(|e| format!("Type error: {}", e))?;

    generate(&ast).map_err(|e| format!("Codegen error: {}", e))
}

#[test]
fn test_empty_list_pattern() {
    let src = r#"
    fun main: () -> Int32 = {
        val lst: List<Int32> = [];
        val result = lst match {
            [] => { 42 }
            _ => { 0 }
        };
        result
    }
    "#;

    let wat = compile_simple(src).unwrap();
    assert!(wat.contains("i32.load"));
}

#[test]
fn test_cons_pattern() {
    let src = r#"
    fun main: () -> Int32 = {
        val lst = [10, 20, 30];
        val result = lst match {
            [] => { 0 }
            [head | tail] => { head }
            _ => { -1 }
        };
        result
    }
    "#;

    let wat = compile_simple(src).unwrap();
    assert!(wat.contains("i32.gt_u ;; length > 0"));
}

#[test]
fn test_exact_pattern() {
    let src = r#"
    fun main: () -> Int32 = {
        val lst = [1, 2, 3];
        val result = lst match {
            [] => { 0 }
            [a] => { a }
            [a, b] => { a + b }
            [a, b, c] => { a + b + c }
            _ => { -1 }
        };
        result
    }
    "#;

    let wat = compile_simple(src).unwrap();
    assert!(wat.contains("i32.const 3"));
}
